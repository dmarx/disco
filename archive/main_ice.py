...
if google_drive is True:
    drive.mount('/home/twmmason/dev/disco/content/drive')
    root_path = '/home/twmmason/dev/disco/content/drive/MyDrive/AI/Disco_Diffusion'
else:
    root_path = '/home/twmmason/dev/disco/content'
...
if google_drive and not yes_please or not google_drive:
    model_path = '/home/twmmason/dev/disco/content/models'
...
if google_drive is not True:
    root_path = f'/home/twmmason/dev/disco/content'
    model_path = '/home/twmmason/dev/disco/content/models'
...
# %cd '/home/twmmason/dev/disco/content/latent-diffusion'
# from ldm.models.diffusion.ddim import DDIMSampler
# %cd '/home/twmmason/dev/disco/content'
    ...
    def forward(self, input):
        ...
        for ch in range(self.cutn):
            if ch > self.cutn - self.cutn//4:
                ...
                size = int(max_size * torch.zeros(1,).normal_(mean=.8,
                           std=.3).clip(float(self.cut_size/max_size), 1.))
...
class MakeCutoutsDango(nn.Module):
    ...
    def forward(self, input):
        ...
        pad_input = F.pad(input, ((sideY-max_size)//2, (sideY-max_size) //
                          2, (sideX-max_size)//2, (sideX-max_size)//2), **padargs)
        ...
        if self.Overview > 0:
            ...
            if cutout_debug:
                TF.to_pil_image(cutouts[0].clamp(0, 1).squeeze(0)).save(
                    "/home/twmmason/dev/disco/content/cutout_overview0.jpg", quality=99)

        if self.InnerCrop > 0:
            for i in range(self.InnerCrop):
                size = int(torch.rand([])**self.IC_Size_Pow *
            ...
            if cutout_debug:
                TF.to_pil_image(cutouts[-1].clamp(0, 1).squeeze(0)).save(
                    "/home/twmmason/dev/disco/content/cutout_InnerCrop.jpg", quality=99)
                    ...
                    for i in range(args.cutn_batches):
                        ...
                        cuts = MakeCutoutsDango(input_resolution,
                                                ...
                                                InnerCrop=args.cut_innercut[1000 -
                                                                            t_int], IC_Size_Pow=args.cut_ic_pow, IC_Grey_P=args.cut_icgray_p[1000-t_int]
                                                )
...
def sr_run(model, img, task, custom_steps, eta, resize_enabled=False, classifier_ckpt=None, global_step=None):
    # global stride
    print("sr run",model, img, task, custom_steps, eta, resize_enabled=False, classifier_ckpt=None, global_step=None)
    ...
    for n in range(n_runs):
        if custom_shape is not None:
            x_T = torch.randn(
                1, custom_shape[1], custom_shape[2], custom_shape[3]).to(model.device)
...
def do_superres(img, filepath):
    ...
    if downsample_rate != 1:
        ...
        # im_og.save('/home/twmmason/dev/disco/content/temp.png')
        # filepath = '/home/twmmason/dev/disco/content/temp.png'

    logs = sr_run(sr_model["model"], im_og, sr_diffMode,
                  sr_diffusion_steps, sr_eta)
    ...
    if downsample_rate != 1:
        ...
        a = a.resize(
            (width_downsampled_post, height_downsampled_post), aliasing)
...
# width_height = [640, 384]  # @param{type: 'raw'}
# width_height = [512, 512]#@param{type: 'raw'}
clip_guidance_scale = cgs  # @param{type: 'number'}
tv_scale = 0  # @param{type: 'number'}
range_scale = rs  # @param{type: 'number'}
sat_scale = 0  # @param{type: 'number'}
cutn_batches = cn_bat  # @param{type: 'number'}
skip_augs = True  # @param{type: 'boolean'}

# @markdown ---

# @markdown ####**Init Settings:**
# "/home/twmmason/dev/disco/content/init_images/woman.jpg"  #None #@param{type: 'string'}
init_image = None #"/home/twmmason/dev/disco/content/init_images/falcor.jpeg"
...
# @param {type: 'string'}
video_init_path = "/home/twmmason/dev/disco/content/training.mp4"
extract_nth_frame = 2  # @param {type:"number"}

if animation_mode == "Video Input":
    videoFramesFolder = f'/home/twmmason/dev/disco/content/videoFrames'
...
# @markdown `zoom` is a do_ of dimensions, 1 is no zoom.
...
text_prompts = {
    0: ["Tianzi Mountains in China, Hyperrealistic, RTX on"],
    100: ["This set of prompts start at frame 100", "This prompt has weight five:5"],
}

text_prompts = {
    0: ["The Big Sur Coast, by Asher Brown Durand, featured on ArtStation"],
    100: ["This set of prompts start at frame 100", "This prompt has weight five:5"],
}
# 0: ["A portal to another dimension in the cupboard in a room with minimalist interior, matte painting trending on artstation"],

# text_prompts = {
#     # 0: ["A magical and surreal picture of a mysterious woman lying in hundreds of snakes in the tropical jungle, by Prateep Kochabua, featured on artstation"],
#     # 0: ["falkor the luck dragon, neverending story, flying in the clouds, artstation in matte painting"],
#     0: ["Alien dreamscape in a futuristic city at night, by Vincent di Fate, trending on artstation"],
#     100: ["This set of prompts start at frame 100", "This prompt has weight five:5"],
# }

# text_prompts = {
#     # 0: ["A magical and surreal picture of a mysterious woman lying in hundreds of snakes in the tropical jungle, by Prateep Kochabua, featured on artstation"],
#     0: ["Fractal geometry starship, galaxy cluster, by Daniel Ljunggren, featured on ArtStation"],
#     100: ["This set of prompts start at frame 100","This prompt has weight five:5"],
# }

text_prompts = {
    # 0: ["A magical and surreal picture of a mysterious woman lying in hundreds of snakes in the tropical jungle, by Prateep Kochabua, featured on artstation"],
    0: ["Glacial ice sheets in the arctic tundra with blue sky, by Daniel Ljunggren, featured on ArtStation"],
    100: ["This set of prompts start at frame 100","This prompt has weight five:5"],
}



# Alien dreamscape, by James Gurney, featured on ArtStation

#

# text_prompts = {
#     0: ["The Isle of Skye, by Asher Brown Durand, featured on ArtStation, RTX on"],
#     100: ["This set of prompts start at frame 100","This prompt has weight five:5"],
# }
...
if resume_run:
    if run_to_resume == 'latest':
        try:
            ...
            batchNum = len(
                glob(f"{batchFolder}/{batch_name}(*)_settings.txt"))-1
    else:
