...
settings = {
    ...
    #'prompt': "A scenic view of marshland dunes, Hyperrealistic, by David Noton, RTX on, matte painting trending on artstation HQ.", #matte painting trending
    'prompt': "A scenic view underwater of large sea monsters and volumetric light, by David Noton and Asher Brown Durand, matte painting trending on artstation HQ.", #matte painting trending
...
max_frames = settings['total_frames']#@param {type:"number"}
...
for i in range(settings['total_frames']):
    ...
    theta =  (math.pi / 180)
    p1 = np.array([px,pz])
    p2 = np.array([r*math.sin(theta),-r*math.cos(theta)])
    tv=  np.tan(np.flip(p1.copy())) #np.flip(np.tan(p1))
    newpos = np.add(p1.copy(),np.multiply(tv.copy(),(math.pi*r*2)/settings['total_frames']))
    ...
    #dx = x2 - px
    #dz = z2 - pz
    # tv = np.tan(px,py) #.arcsin( math.sin(r) / math.cos(theta))
    ...
    #px += dx
    #pz += dz
    sr= str(i) +": (" + str((theta/4)) + "),"
    #sa += str(i) +": (-" + str( (math.pi / 180)/8) + "),"

interp_spline = 'Linear' #Do not change, currently will not look good. param ['Linear','Quadratic','Cubic']{type:"string"}
angle = "0:(1.0)" #sa #@param {type:"string"}
...
translation_z = sz #"0: (0.0)" #0: (-10.0), 360: (-10)" #"0: (0.0)" #0: (1.0),360: (1.0)" # #"0: (10.0)"#@param {type:"string"}
rotation_3d_x = "0: (0)"#@param {type:"string"}
rotation_3d_y = sr #"0: (0)" ##@param {type:"string"}
rotation_3d_z = "0: (0)"#@param {type:"string"}
midas_depth_model = "dpt_large"#@param {type:"string"}
midas_weight = 0.3#@param {type:"number"}
near_plane = 20#@param {type:"number"}
