...
from disco.prompt import get_inbetweens, parse_key_frames


import generator.common.envutils as envutils
envutils.configure_sys_paths(os.getcwd(),os.getcwd()+"/content/models",True)

from generator.common import models, utils
from generator.common.cutouts import MakeCutouts, MakeCutoutsDango
...
class Generator:
    ...
    def do_run(self):
        ...
        if (args.animation_mode == "3D") and (args.midas_weight > 0.0):
            midas_model, midas_transform, midas_net_w, midas_net_h, midas_resize_mode, midas_normalization = utils.init_midas_depth_model(args.midas_depth_model)
        for frame_num in range(args.start_frame, args.max_frames):
            ...
            if args.animation_mode == "2D":
                ...
                if frame_num > 0:
                    seed += 1
                    if self.resume_run and frame_num == self.start_frame:
            ...
            if args.animation_mode == "3D":
                if frame_num > 0:
                    ...
                    if self.resume_run and frame_num == self.start_frame:
                        ...
                        if self.turbo_mode and frame_num > self.turbo_preroll:
                            shutil.copyfile(img_filepath, 'oldFrameScaled.png')
                    else:
                        img_filepath = 'prevFrame.png'

                    next_step_pil = utils.do_3d_step(img_filepath, frame_num, midas_model, midas_transform)
                    next_step_pil.save('prevFrameScaled.png')

                    ### Turbo mode - skip some diffusions, use 3d morph for clarity and to save time
                    if self.turbo_mode:
                        if frame_num == self.turbo_preroll: #start tracking oldframe
                            next_step_pil.save('oldFrameScaled.png')#stash for later blending          
                        elif frame_num > self.turbo_preroll:
                        #set up 2 warped image sequences, old & new, to blend toward new diff image
                            old_frame = utils.do_3d_step('oldFrameScaled.png', frame_num, midas_model, midas_transform)
                            old_frame.save('oldFrameScaled.png')
                            if frame_num % int(self.turbo_steps) != 0: 
                                print('turbo skip this frame: skipping clip diffusion steps')
                                filename = f'{args.batch_name}({args.batchNum})_{frame_num:04}.png'
                                blend_factor = ((frame_num % int(self.turbo_steps))+1)/int(self.turbo_steps)
            ...
            if  args.animation_mode == "Video Input":
                if not self.video_init_seed_continuity:
                    seed += 1
                    init_image = f'{self.videoFramesFolder}/{frame_num+1:04}.jpg'
                    init_scale = args.frames_scale
                    skip_steps = args.calc_frames_skip_steps
            ...
            for clip_model in self.clip_models:
                ...
                if image_prompt:
                    ...
                    for prompt in image_prompt:
                        ...
                        embed = clip_model.encode_image(utils.normalize(batch)).float()
                ...
                with torch.enable_grad():
                    ...
                    for model_stat in model_stats:
                        for i in range(args.cutn_batches):
                            ...
                            clip_in = utils.normalize(cuts(x_in.add(1).div(2)))
                                ...
                                for k, image in enumerate(sample['pred_xstart']):
                                    ...
                                    if args.n_batches > 0:
                                        ...
                                        if cur_t == -1 and args.intermediates_in_subfolder is True:
                                            save_num = f'{frame_num:04}' if self.animation_mode != "None" else i
                                        ...
                                        if frame_num == 0:
                                            ...
                                            if args.animation_mode != "None":
                                                ...
                                                # If turbo, save a blended image
                                                    if self.turbo_mode and frame_num > 0:
    ...
    def init_run(self):
        ...
        if resume_run:
            if run_to_resume == 'latest':
                try:
                    ...
                    batchNum = len(glob(f"{self.batchFolder}/{self.batch_name}(*)_settings.txt"))-1
            else:
                batchNum = int(run_to_resume)
            if resume_from_frame == 'latest':
                self.start_frame = len(glob(self.batchFolder+f"/{self.batch_name}({batchNum})_*.png"))
                ...
                if self.animation_mode != '3D' and self.turbo_mode == True and self.start_frame > self.turbo_preroll and self.start_frame % int(self.turbo_steps) != 0:
                    self.start_frame = self.start_frame - (self.start_frame % int(self.turbo_steps))
                if retain_overwritten_frames is True:
                    existing_frames = len(glob(self.batchFolder+f"/{self.batch_name}({batchNum})_*.png"))
        ...
        print(f'Starting Run: {self.batch_name}({batchNum}) at frame {self.start_frame}')
    ...
    def init_variables(self):
        ...
        clip_models = []
        if ViTB32 is True: clip_models.append(clip.load('ViT-B/32', jit=False)[0].eval().requires_grad_(False).to(self.device)) 
        if ViTB16 is True: clip_models.append(clip.load('ViT-B/16', jit=False)[0].eval().requires_grad_(False).to(self.device) ) 
        if ViTL14 is True: clip_models.append(clip.load('ViT-L/14', jit=False)[0].eval().requires_grad_(False).to(self.device) ) 
        if RN50 is True: clip_models.append(clip.load('RN50', jit=False)[0].eval().requires_grad_(False).to(self.device))
        if RN50x4 is True: clip_models.append(clip.load('RN50x4', jit=False)[0].eval().requires_grad_(False).to(self.device)) 
        if RN50x16 is True: clip_models.append(clip.load('RN50x16', jit=False)[0].eval().requires_grad_(False).to(self.device)) 
        if RN50x64 is True: clip_models.append(clip.load('RN50x64', jit=False)[0].eval().requires_grad_(False).to(self.device)) 
        if RN101 is True: clip_models.append(clip.load('RN101', jit=False)[0].eval().requires_grad_(False).to(self.device)) 
        ...
        self.max_frames = settings['max_frames'] #@param {type:"number"}
        ...
        if self.key_frames:
            try:
                self.angle_series = get_inbetweens(parse_key_frames(self.angle))
            ...
            try:
                self.zoom_series = get_inbetweens(parse_key_frames(self.zoom))
            except RuntimeError as e:
                print(
                    "WARNING: You have selected to use key frames, but you have not "
            ...
            try:
                self.translation_x_series = get_inbetweens(parse_key_frames(self.translation_x))
            except RuntimeError as e:
                print(
                    "WARNING: You have selected to use key frames, but you have not "
            ...
            try:
                self.translation_y_series = get_inbetweens(parse_key_frames(self.translation_y))
            except RuntimeError as e:
                print(
                    "WARNING: You have selected to use key frames, but you have not "
            ...
            try:
                self.translation_z_series = get_inbetweens(parse_key_frames(self.translation_z))
            except RuntimeError as e:
                print(
                    "WARNING: You have selected to use key frames, but you have not "
            ...
            try:
                self.rotation_3d_x_series = get_inbetweens(parse_key_frames(self.rotation_3d_x))
            except RuntimeError as e:
                print(
                    "WARNING: You have selected to use key frames, but you have not "
            ...
            try:
                self.rotation_3d_y_series = get_inbetweens(parse_key_frames(self.rotation_3d_y))
            except RuntimeError as e:
                print(
                    "WARNING: You have selected to use key frames, but you have not "
            ...
            try:
                self.rotation_3d_z_series = get_inbetweens(parse_key_frames(self.rotation_3d_z))
            except RuntimeError as e:
                print(
                    "WARNING: You have selected to use key frames, but you have not "
        ...
        if self.intermediate_saves and self.intermediates_in_subfolder is True:
            ...
            self.set_seed = '3545594394' #'random_seed' #@param{type: 'string'}
            ...
            self.display_rate =  50 #@param{type: 'number'}
            self.n_batches =  50 #@param{type: 'number'}
    ...
    def save_settings(self):
        setting_list = {
            ...
            'cut_innercut': self.str(self.cut_innercut),
            'cut_ic_pow': self.cut_ic_pow,
            'cut_icgray_p': self.str(self.cut_icgray_p),
            'key_frames': self.key_frames,
            'max_frames': self.max_frames,
            'angle': self.angle,
            'zoom': self.zoom,
