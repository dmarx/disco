...
#         root_path = '/content/drive/MyDrive/AI/Centipede_Diffusion'
#     else:
#         root_path = '/content'
# else:
root_path = os.getcwd() +  "/content"
...
outputs_path = f'{outDirPath}/latent_res' # latent diffusion results
#createPath(outputs_path)
...
print(f"Outputs will be saved to {outDirPath}")

# libraries = f'{root_path}/libraries'
# createPath(libraries)

#@title ### 1.3.1 Install and import dependencies
...
# Latent Diffusion dependencies
gitclone("https://github.com/crowsonkb/latent-diffusion.git")
gitclone("https://github.com/CompVis/taming-transformers")
subprocess.run(['pip', 'install', '-e', './taming-transformers'], stdout=subprocess.PIPE).stdout.decode('utf-8')

multipip_res = subprocess.run(['pip', 'install', 'lpips', 'datetime', 'timm', 'ftfy',
                               'omegaconf>=2.0.0', 'pytorch-lightning>=1.0.8', 'torch-fidelity', 'einops',
                               'transformers', 'open_clip_torch', 'autokeras', 'tensorflow'], stdout=subprocess.PIPE).stdout.decode('utf-8')
print(multipip_res)

#sys.path.append(PROJECT_DIR)
sys.path.append(f'{PROJECT_DIR}/taming-transformers')
...
#@title ### 1.4.1 Latent Diffusion model
# %cd /content/latent-diffusion

if os.path.isfile(f"{model_path}/latent_diffusion_txt2img_f8_large.ckpt"):
  print("Using saved model from Google Drive")
else:
  subprocess.run(['wget', '-O', f"{model_path}/latent_diffusion_txt2img_f8_large.ckpt", "https://ommer-lab.com/files/latent-diffusion/nitro/txt2img-f8-large/model.ckpt"], stdout=subprocess.PIPE).stdout.decode('utf-8')
...
# Commented out IPython magic to ensure Python compatibility.
#@title 1.4.2 Define necessary functions (Latent Diffusion)


# def load_safety_model(clip_model_ld):
...
#     cache_folder = home + "/.cache/clip_retrieval/" + clip_model_ld.replace("/", "_")
#     if clip_model_ld == "ViT-L/14":
#         model_dir = cache_folder + "/clip_autokeras_binary_nsfw"
#         dim = 768
#     elif clip_model_ld == "ViT-B/32":
...
#             )
#         else:
#             raise ValueError("Unknown model {}".format(clip_model_ld))
...
# clip_model_ld, _, preprocess = open_clip.create_model_and_transforms('ViT-B-32', pretrained='openai')
...
device_ld = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
        ...
        with torch.cuda.amp.autocast():
            with model_ld.ema_scope():
                ...
                for n in trange(opt.n_iter, desc="Sampling"):
                    c = model_ld.get_learned_conditioning(opt.n_samples * [prompt])
                    ...
                    for x_sample in x_samples_ddim:
                        ...
                        if(not unsafe):
                          image_vector.save(os.path.join(sample_path, f"{base_count}.png"))
                        else:
...
#@title ### 1.5.1 Define Midas functions
...
#@title 1.5.2 Define necessary functions (Disco Diffusion)
...
def do_run():
  ...
  for frame_num in range(args.start_frame, args.max_frames):
      ...
      for i in range(args.n_batches):
          if args.animation_mode == 'None':
            display.clear_output(wait=True)
            batchBar = tqdm(range(args.n_batches_out), desc ="Batches")
            batchBar.n = args.cur_batch_out
            batchBar.refresh()
          print('')
          ...
          #if args.latent_res_inits is not None and args.animation_mode == 'None':
          #    init_image = args.latent_res_inits[i]
          #    init = Image.open(fetch(init_image)).convert('RGB')
          #    init = init.resize((args.side_x, args.side_y), Image.LANCZOS)
          #    init = TF.to_tensor(init).to(device).unsqueeze(0).mul(2).sub(1)
                  ...
                  for k, image in enumerate(sample['pred_xstart']):
                      ...
                      if args.n_batches > 0:
                        ...
                        if cur_t == -1 and args.intermediates_in_subfolder is True:
                          save_num = f'{frame_num:04}' if animation_mode != "None" else args.cur_batch_out
                          filename = f'{args.batch_name}({args.batchNum})_{save_num}.png'
                        else:
                          #If we're working with percentages, append it
                          if args.steps_per_checkpoint is not None:
                            filename = f'{args.batch_name}({args.batchNum})_{args.cur_batch_out:04}-{percent:02}%.png'
                          # Or else, iIf we're working with specific steps, append those
                          else:
                            filename = f'{args.batch_name}({args.batchNum})_{args.cur_batch_out:04}-{j:03}.png'
...
def save_settings():
  setting_list = {
    'prompt_1 (ld)' : prompt_1,
    'Steps (ld)' : Steps,
    'ETA (ld)' : ETA,
    'Diversity_scale (ld)' : Diversity_scale,
    'PLMS_sampling (ld)' : PLMS_sampling,
    'prompt_2': prompt_2,
    'text_prompts': text_prompts,
    'image_prompts': image_prompts,
    'clip_guidance_scale_b': clip_guidance_scale_b,
    'tv_scale': tv_scale,
    'range_scale': range_scale,
    'sat_scale': sat_scale,
    # 'cutn': cutn,
    'cutn_batches_b': cutn_batches_b,
    'max_frames': max_frames,
    'interp_spline': interp_spline,
    # 'rotation_per_frame': rotation_per_frame,
    'latent_res_inits': latent_res_inits,
    #'init_image': init_image,
    'init_scale_b': init_scale_b,
    'skip_steps_b': skip_steps_b,
...
#@markdown ##**Latent Diffusion Settings:**

prompt_1 = "A view of Elephants drinking from a swimming pool outside a suburban house, by Felix Kahn, matte painting trending on artstation HQ." #@param{type:"string"}
#@markdown prompt_1 define the global shape of the desired result.
Steps = 250 #@param {type:"integer"}
ETA =  0#@param{type:"integer"}
Iterations =  3#@param{type:"integer"}
Width=192 #@param{type:"integer"}
...
#@markdown ##**Disco Diffusion Settings:**
#@markdown #####For "prompt_2" parameters suffixed with "_b", you can set multiple values ​​in a list. Each will apply to the current element of the batch. If the batch is larger than the number of values ​​defined for a parameter, the last element in the list will apply to the following elements.
#@markdown ####**Basic Settings:**
prompt_2 = ["A view of Elephants drinking from a swimming pool outside a suburban house, by Felix Kahn, matte painting trending on artstation HQ."] #@param{type: 'raw'}
#@markdown prompt_2 define the style, skin and details of the desired result.
#@markdown #####*By setting multiple prompts, you can have one per input of the batch.*
#@markdown #####**For animations, you still have to modify text_prompts in the code to use multiple prompts.**
...
batch_name = 'TimeToLatentDisco' #@param{type: 'string'}
steps_b = [300] #@param{type: 'raw'}

width_height_mult = 4.0 #@param{type: 'number'}
width_height = [int(Width*width_height_mult), int(Height*width_height_mult)]

clip_guidance_scale_b = [20000] #@param{type: 'raw'}
tv_scale =  0#@param{type: 'number'}
range_scale =   150#@param{type: 'number'}
sat_scale =   0#@param{type: 'number'}
cutn_batches_b =   [4]#@param{type: 'raw'}
...
init_scale_b =  [500]#@param{type: 'raw'}
skip_steps_b = [150] #@param{type: 'raw'}
...
#@title ##**4.1 Run Latent Diffusion !**
import argparse
...
#@title ##**4.2 Run Disco Diffusion !**

skip_disco_diffusion = False #@param{type: 'boolean'}
#@markdown Manually check Latent Diffusion results before running Disco Diffusion. DON'T FORGET TO UNCHECK!

del model_ld
torch.cuda.empty_cache()
gc.collect()

if not skip_disco_diffusion:
  from glob import glob
  
  prompts_2_b = prompt_2.copy()
  if len(prompts_2_b) == 0:
    prompts_2_b = [""]
  use_prompts_2_b = False
  if len(text_prompts) == 0:
    use_prompts_2_b = True

  latent_res_inits_idx =  [4, 6, 6]#@param{type: 'raw'}
  #@markdown Give the names of the desired results of the previous step to batch (eg: [0, 0, 2, 3]). An empty list means all results. None means run only Disco Diffusion. Only the first result is taken with animation modes.

  n_batches_u = 1 #@param{type: 'integer'}
  #@markdown `n_batches_u` ignored with animation modes or if there is latent diffusion results as init images.
  n_batches = n_batches_u

  latent_res_inits = []
  if latent_res_inits_idx is not None:
    if len(latent_res_inits_idx) == 0 :
      latent_res_inits = list(range(Samples_in_parallel*Iterations))
    else :
      latent_res_inits = latent_res_inits_idx.copy()
    
    n_batches = len(latent_res_inits)

    latent_res_inits = [outputs_path+"/samples/"+str(i)+".png" for i in latent_res_inits]
  
  if animation_mode != "None":
    n_batches = 1

  latent_res_inits = reach(latent_res_inits, n_batches)
  prompts_2_b = reach(prompts_2_b, n_batches)
  steps_bc = reach(steps_b, n_batches)
  clip_guidance_scale_bc = reach(clip_guidance_scale_b, n_batches)
  cutn_batches_bc = reach(cutn_batches_b, n_batches)
  init_scale_bc = reach(init_scale_b, n_batches)
  skip_steps_bc = reach(skip_steps_b, n_batches)
  ...
  for bidx in range(n_batches) :
    steps = steps_bc[bidx]
    clip_guidance_scale = clip_guidance_scale_bc[bidx]
    cutn_batches = cutn_batches_bc[bidx]
    init_scale = init_scale_bc[bidx]
    skip_steps = skip_steps_bc[bidx]

    if use_prompts_2_b:
      text_prompts[0] = [prompts_2_b[bidx]]
    
    if len(latent_res_inits) != 0:
      init_image = latent_res_inits[bidx]
