...
#       'prompt':"A scenic view of a mystical place, by David Noton and by Asher Brown Durand, detailed render, I can't believe how detailed this is, matte painting trending on artstation artstation HQ.",
   ...
   'clip_guidance_scale':25000, #5000,#A wide-angle fisheye lens view of a modernist minimalist interior, fisheye lens effect
    'steps':200,
    'cut_ic_pow':1,
    'range_scale':250,
    'n_batches':5,
    ...
    'skip_augs':False,
    'frames_scale': 25000, #@param{type: 'integer'}
...
#@markdown ---
#@markdown ####**Softology Clamp Fix Test:**
#@markdown *Experimental! ...may help reduce fading / clipping in animations. #3 seems the best.*

remove_clamp_1 = False#@param{type: 'boolean'}
remove_clamp_2 = False#@param{type: 'boolean'}
remove_clamp_3 = False#@param{type: 'boolean'}
...
def range_loss(input):
    #Softology Clamp fix #1
    if args.remove_clamp_1 == 0:
...
def do_run():
    ...
    for frame_num in range(args.start_frame, args.max_frames):
        ...
        if args.animation_mode == "2D":
            if args.key_frames:
                ...
                if frame_num > 0:
                    ...
                    img_0 = cv2.imread(settings['path'] + '/content/prevFrame.png')
                ...
                cv2.imwrite(settings['path'] + '/content/prevFrameScaled.png', img_0)
                init_image = settings['path'] + '/content/prevFrameScaled.png'
        ...
        if args.animation_mode == "3D":
            ...
            if frame_num > 0:
                seed = seed + 1      ### Turbo mode prep
                turbo_prevScaled_path = batchFolder+f"/turbo/prevFrameScaled.png"
                turbo_oldScaled_path = batchFolder+f"/turbo/oldFrameScaled.png" 
                turbo_preroll = 10 # frames
                non_turbo_prevFrm = settings['path']  + '/content/prevFrame.png'
                if resume_run and frame_num == start_frame:
                    ...
                    img_filepath = settings['path']  + '/content/prevFrame.png' 
                ...
                for i in range(3):
                    ...
                    next_step_pil = dxf.transform_image_3d(img_filepath, midas_model, midas_transform, DEVICE,
                                                                    ...
                                                                    sampling_mode=args.sampling_mode, midas_weight=args.midas_weight,spherical=(i!=2))
                    if i==2: 
                        next_step_pil.save(settings['path']+'/content/prevFrameScaled.png')
                    else:
...
#Softology clamp fix #2
                if args.remove_clamp_2 == 0:
                ...
                with image_display:
                    if j % args.display_rate == 0 or cur_t == -1 or intermediateStep == True:
                        for k, image in enumerate(sample['pred_xstart']):
                            ...
                            #Softology clamp fix #3
                            if args.remove_clamp_3 == 0:
                            ...
                            if cur_t == -1:
                                ...
                                if frame_num == 0:
                                    save_settings()
                                if args.animation_mode != "None":
                                    image.save(settings['path'] + '/content/prevFrame.png')
                                print("sharpen_preset",args.sharpen_preset)
                                if args.sharpen_preset != "Off" and animation_mode == "None":
                                    ...
                                    print("turbo_mode",turbo_mode)
                                    #if turbo_mode, save a blended image 
                                    if turbo_mode == True and len(turbo_prevScaled_path) > 0:
                                        #mix new image with prevFrameScaled
                                        blend_factor = (1)/int(turbo_steps)
                                        newFrame = cv2.imread(settings['path']+'/content/prevFrame.png')#this got updated just above..
...
def save_settings():
  setting_list = {
    ...
    'turbo_steps':turbo_steps,
      'remove_clamp_1': remove_clamp_1,
    'remove_clamp_2': remove_clamp_2,
    'remove_clamp_3': remove_clamp_3,  }
...
init_image = None #root_path + "/content/init_images/init-equi-square.jpg" #@param{type: 'string'}
init_scale = 1000 #@param{type: 'integer'}
skip_steps = settings['skip_steps'] #@param{type: 'integer'}
#@markdown *Make sure you set skip_steps to ~50% of your steps if you want to use an init image.*
print("set skip_steps")
...
if is_colab:
    video_init_path = settings['path']  +"/content/training.mp4" #@param {type: 'string'}
else:
...
if animation_mode == "Video Input":
  if is_colab:
      videoFramesFolder =settings['path']  + f'/content/videoFrames'
  else:
...
#Turbo
cut_overview = "[8]*30+[0]*970" #@param {type: 'string'}       
cut_innercut ="[8]*30+[16]*970"#@param {type: 'string'}  
cut_ic_pow = 1#@param {type: 'number'}  
cut_icgray_p = "[0.2]*30+[0]*970"#@param {type: 'string'}
