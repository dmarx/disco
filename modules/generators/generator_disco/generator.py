...
import modules.generators.generator_disco.common.envutils as envutils
...
from modules.generators.generator_disco.common.cutouts import (
    MakeCutouts,
    MakeCutoutsDango,
)
...
class GeneratorDisco(GeneratorBase):
    ...
    settings = {
        ...
        "text_prompts": [
            [
                0,
                "A scenic view of a beautiful tropical beach by David Noton, bright sunny day",
            ]
        ],
        ...
        "soft_limiter_knee": 0.98,  # @param{type: 'number'}\n",
        ...
        for self.frame_num in range(args.start_frame, args.max_frames):
            ...
            if args.animation_mode == "2D":
                ...
                if self.frame_num > 0:
                    ...
                    if self.resume_run and self.frame_num == self.start_frame:
                        img_0 = cv2.imread(
                            self.batchFolder
                            + f"/{self.batch_name}({self.batchNum})_{self.start_frame-1:04}.png"
                        )
                    else:
            ...
            if args.animation_mode == "3D":
                if self.frame_num > 0:
                    ...
                    next_step_pil = utils.do_3d_step(
                        img_filepath,
                        self.frame_num,
                        midas_model,
                        midas_transform,
                    ...
                    if self.turbo_mode:
                        if (
                            self.frame_num == self.turbo_preroll
                        ):  # start tracking oldframe
                            next_step_pil.save(
                                "content/oldFrameScaled.png"
                            )  # stash for later blending
                        elif self.frame_num > self.turbo_preroll:
                            # set up 2 warped image sequences, old & new, to blend toward new diff image
                            old_frame = utils.do_3d_step(
                                "content/oldFrameScaled.png",
                                self.frame_num,
                                midas_model,
                                midas_transform,
                            ...
                            if self.frame_num % int(self.turbo_steps) != 0:
                                ...
                                cv2.imwrite(
                                    f"{self.batchFolder}/{filename}", blendedImage
                                )
                                next_step_pil.save(
                                    f"{img_filepath}"
                                ...
                                cv2.imwrite(
                                    f"content/oldFrameScaled.png", oldWarpedImg
            ...
            if args.prompts_series is not None and self.frame_num >= len(
                args.prompts_series
            ):
            ...
            if args.image_prompts_series is not None and self.frame_num >= len(
            ...
            for clip_model in self.clip_models:
                ...
                if image_prompt:
                    ...
                    for prompt in image_prompt:
                        ...
                        img = TF.resize(
                            img,
                            min(self.side_x, self.side_y, *img.size),
                            T.InterpolationMode.LANCZOS,
                        )
                        batch = model_stat["make_cutouts"](
                            TF.to_tensor(img).to(self.device).unsqueeze(0).mul(2).sub(1)
                        )
            ...
            if args.perlin_init:
                if args.perlin_mode == "color":
                    init = utils.create_perlin_noise(
                        [1.5 ** -i * 0.5 for i in range(12)], 1, 1, False
                    )
                    init2 = utils.create_perlin_noise(
            ...
            def cond_fn(x, t, y=None):
                with torch.enable_grad():
                    ...
                    if self.use_secondary_model is True:
                        alpha = torch.tensor(
                            self.diffusion.sqrt_alphas_cumprod[cur_t],
                            device=self.device,
                            dtype=torch.float32,
                        )
                        sigma = torch.tensor(
                    ...
                    for model_stat in model_stats:
                        for i in range(args.cutn_batches):
                            ...
                            dists = utils.spherical_dist_loss(
                            ...
                            x_in_grad += (
                                torch.autograd.grad(
                                    losses.sum() * self.clip_guidance_scale, x_in
                                )[0]
                                / self.cutn_batches
                            )
            ...
            for i in range(args.n_batches):
                ...
                for j, sample in enumerate(samples):
                    if self.chain.project != None:
                        self.chain.progress += (1.0 / self.steps) / len(
                        ...
                        if (
                            ...
                            for k, image in enumerate(sample["pred_xstart"]):
                                ...
                                if j % args.display_rate == 0 or cur_t == -1:
                                    ...
                                    os.system(
                                        'cp content/progress.png "'
                                        + os.getcwd()
                                        + '/static/output/progress.png"'
                                    )
                                if args.steps_per_checkpoint is not None:
                                    if j % args.steps_per_checkpoint == 0 and j > 0:
                                        if args.intermediates_in_subfolder is True:
                                            image.save(
                                                f"{self.partialFolder}/{filename}"
                                            )
                                        else:
    ...
    def do_run(self):
        ...
        if self.resume_run:
            if run_to_resume == "latest":
                try:
                    ...
                    self.batchNum = (
                        len(
                            glob(
                                f"{self.batchFolder}/{self.batch_name}(*)_settings.txt"
                            )
                        )
                        - 1
                    )
            ...
            if resume_from_frame == "latest":
                self.start_frame = len(
                    glob(
                        self.batchFolder + f"/{self.batch_name}({self.batchNum})_*.png"
                    )
                )
                if (
                    self.animation_mode != "3D"
                    and self.turbo_mode == True
                    and self.start_frame > self.turbo_preroll
                    and self.start_frame % int(self.turbo_steps) != 0
                ):
                    self.start_frame = self.start_frame - (
                        self.start_frame % int(self.turbo_steps)
                    )
            else:
                ...
                if retain_overwritten_frames is True:
                    existing_frames = len(
                        glob(
                            self.batchFolder
                            + f"/{self.batch_name}({self.batchNum})_*.png"
                        )
                    )
                    frames_to_save = existing_frames - self.start_frame
                    self.chain.output_message(
                        f"Moving {frames_to_save} frames to the Retained folder"
                    )
                    self.move_files(
                        self.start_frame,
                        existing_frames,
            ...
            while (
                os.path.isfile(
                    f"{self.batchFolder}/{self.batch_name}({self.batchNum})_settings.txt"
                )
                is True
                or os.path.isfile(
                    f"{self.batchFolder}/{self.batch_name}-{self.batchNum}_settings.txt"
        ...
        self.chain.output_message(
            f"Init Run: {self.batch_name}({self.batchNum}) at frame {self.start_frame}"
        )
        ...
        self.args = {
            ...
            "prompts_series": utils.split_prompts(self.text_prompts, self.max_frames)
            if self.text_prompts
            else None,
            "image_prompts_series": utils.split_prompts(
                self.image_prompts, self.max_frames
            )
    ...
    def init_settings(self, override_settings=None):
        ...
        self.clip_guidance_scale = settings[
            "clip_guidance_scale"
        ]  # @param{type: 'number'}
        ...
        self.soft_limiter_on = settings[
            "soft_limiter_on"
        ]  # @param{type: 'boolean'}\n",
        self.soft_limiter_knee = settings[
            "soft_limiter_knee"
        ]  # @param{type: 'number'}\n",
        if self.soft_limiter_knee < 0.5 or self.soft_limiter_knee > 0.999:
            self.soft_limiter_knee = 0.98
            self.chain.output_message(
                "soft_limiter_knee out of range. Automatically reset to 0.98"
            )
        ...
        if self.animation_mode == "Video Input":
            ...
            subprocess.run(
                [
                    ...
                    "error",
                    "-stats",
                    f"{self.videoFramesFolder}/%04d.jpg",
                ],
        ...
        self.turbo_steps = settings[
            "turbo_steps"
        ]  # @param ["2","3","4","5","6"] {type:"string"}
        ...
        self.frames_skip_steps = settings[
            "frames_skip_steps"
        ]  # @param ['40%', '50%', '60%', '70%', '80%'] {type: 'string'}

        if self.key_frames:
            try:
                self.angle_series = utils.get_inbetweens(
                    key_frames=utils.parse_key_frames(self.angle),
            ...
            try:
                self.zoom_series = utils.get_inbetweens(
                    key_frames=utils.parse_key_frames(self.zoom),
                    max_frames=self.max_frames,
                    interp_spline=self.interp_spline,
                )
            except RuntimeError as e:
            ...
            try:
                self.translation_x_series = utils.get_inbetweens(
                    key_frames=utils.parse_key_frames(self.translation_x),
                    max_frames=self.max_frames,
                    interp_spline=self.interp_spline,
                )
            except RuntimeError as e:
            ...
            try:
                self.translation_y_series = utils.get_inbetweens(
                    key_frames=utils.parse_key_frames(self.translation_y),
                    max_frames=self.max_frames,
                    interp_spline=self.interp_spline,
                )
            except RuntimeError as e:
            ...
            try:
                self.translation_z_series = utils.get_inbetweens(
                    key_frames=utils.parse_key_frames(self.translation_z),
                    max_frames=self.max_frames,
                    interp_spline=self.interp_spline,
                )
            except RuntimeError as e:
            ...
            try:
                self.rotation_3d_x_series = utils.get_inbetweens(
                    key_frames=utils.parse_key_frames(self.rotation_3d_x),
                    max_frames=self.max_frames,
                    interp_spline=self.interp_spline,
                )
            except RuntimeError as e:
            ...
            try:
                self.rotation_3d_y_series = utils.get_inbetweens(
                    key_frames=utils.parse_key_frames(self.rotation_3d_y),
                    max_frames=self.max_frames,
                    interp_spline=self.interp_spline,
                )
            except RuntimeError as e:
            ...
            try:
                self.rotation_3d_z_series = utils.get_inbetweens(
                    key_frames=utils.parse_key_frames(self.rotation_3d_z),
                    max_frames=self.max_frames,
                    interp_spline=self.interp_spline,
                )
            except RuntimeError as e:
        ...
        if type(self.intermediate_saves) is not list:
            if self.intermediate_saves:
                ...
                self.steps_per_checkpoint = (
                    self.steps_per_checkpoint if self.steps_per_checkpoint > 0 else 1
                )
                self.chain.output_message(
                    f"Will save every {self.steps_per_checkpoint} steps"
                )
            else:
        ...
        if self.intermediate_saves and self.intermediates_in_subfolder is True:
            ...
            self.set_seed = (
                "random_seed"  #'3545594394' #'random_seed' #@param{type: 'string'}
            )
    ...
    def load_models(self, settings):
        ...
        self.use_secondary_model = settings[
            "use_secondary_model"
        ]  # @param {type: 'boolean'}
        ...
        model_512_path = (
            f"{self.model_path}/512x512_diffusion_uncond_finetune_008100.pt"
        )
        ...
        if self.diffusion_model == "256x256_diffusion_uncond":
            if os.path.exists(model_256_path) and check_model_SHA:
                ...
                if hash == model_256_SHA:
                    ...
                    self.chain.output_message(
                        "256 Model SHA doesn't match, redownloading..."
                    )
            ...
            if os.path.exists(model_512_path) and check_model_SHA:
                ...
                if hash == model_512_SHA:
                    ...
                    self.chain.output_message(
                        "512 Model SHA doesn't match, redownloading..."
                    )
        ...
        if self.use_secondary_model:
            ...
            secondary_model.load_state_dict(
                torch.load(
                    f"{self.model_path}/secondary_model_imagenet_2.pth",
                    map_location="cpu",
                )
            )
    ...
    def update_model_config(self,settings):
        ...
        self.model.load_state_dict(
            torch.load(
                f"{self.model_path}/{self.diffusion_model}.pt", map_location="cpu"
            )
        )
    ...
    def save_settings(self):
        ...
        with open(
            f"{self.batchFolder}/{self.batch_name}({self.batchNum})_settings.txt", "w+"
        ) as f:  # save settings
    ...
    def create_animation_data(self):
        ...
        for i in range(self.max_frames):
            ...
            newpos = np.add(
                p1.copy(), np.multiply(tv.copy(), (math.pi * r * 2) / frame)
            )
