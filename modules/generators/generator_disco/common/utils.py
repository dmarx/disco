...
import modules.generators.generator_disco.common.disco_xform_utils as dxf
import cv2
...
def init_midas_depth_model(
    midas_model_type="dpt_large", optimize=True, DEVICE=None, default_models=None
):
    midas_model = None
    net_w = None
    net_h = None
...
def create_perlin_noise(
    side_x, side_y, octaves=[1, 1, 1, 1], width=2, height=2, grayscale=True
):
...
def soft_limit(x, soft_limiter_knee):
    ...
    soft_sign = x / abs(x)
    soft_overage = (
        (abs(x) - soft_limiter_knee) + (abs(abs(x) - soft_limiter_knee))
    ) / 2
    soft_base = abs(x) - soft_overage
    soft_limited_x = soft_base + torch.tanh(soft_overage / (1 - soft_limiter_knee)) * (
        1 - soft_limiter_knee
    )
