  ...
  <div class="card">
    ...
    <div class="card-body pt-0">
      ...
      >
        ...
        <template v-slot:cell-name="{ row: customer }">{{ customer.name }}</template>
        <template v-slot:cell-email="{ row: customer }">
          <a href="#" class="text-gray-600 text-hover-primary mb-1">{{ customer.email }}</a>
        </template>
        <template v-slot:cell-company="{ row: customer }">{{ customer.company }}</template>
        ...
        <template v-slot:cell-date="{ row: customer }">{{ customer.date }}</template>
        <template v-slot:cell-actions="{ row: customer }">
          ...
          <!--begin::Menu-->
          <div
            class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-175px py-4"
            data-kt-menu="true"
          >
            <!--begin::Menu item-->
            <div class="menu-item px-3">
              <router-link
                to="/case/attribution/profile-details"
                class="menu-link px-3"
              >View&nbsp;Subject&nbsp;Profile</router-link>
            </div>
            <div class="menu-item px-3">
              <router-link
                to="/case/attribution/profile-attribution"
                class="menu-link px-3"
              >View&nbsp;Attribution</router-link>
            </div>
            <!--end::Menu item-->
            <!--begin::Menu item-->
            <div class="menu-item px-3">
...
export default defineComponent({
  ...
  setup() {
    ...
    onMounted(() => {
      setCurrentPageBreadcrumbs("Attribution", ["Case"]);
